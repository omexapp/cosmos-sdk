...
// omexchain full-node start flags
const (
...
func StopCmd(ctx *Context) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				errStr := fmt.Sprintf("%s Please finish the process of omexchaind through kill -2 pid to stop gracefully", err.Error())
				cmn.Exit(errStr)
			}
			defer f.Close()
...
func registerRestServerFlags(cmd *cobra.Command) *cobra.Command {
	...
	cmd.Flags().String(FlagRestPathPrefix, "omexchain", "Path prefix for registering rest api route.")
...
// registeromexchainPluginFlags registers the flags required for rest server
func registeromexchainPluginFlags(cmd *cobra.Command) *cobra.Command {
