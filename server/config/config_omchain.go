...
var defaultNodeHome = os.ExpandEnv("$HOME/.omexchaind")
...
type BackendOrmEngineInfo struct {
	...
	// if engine_type is sqlite3, it should be a local path, e.g.) /Users/lingting.fu/.omexchaind/data/sqlite3/backend.db
...
// StreamConfig - config for omchain stream module
...
// DefaultStreamConfig returns default config for omchain stream module
